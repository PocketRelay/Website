...
:::warning less supported
This version of **Pocket Relay** is less supported than the Plugin Client this is mentioned in the [Joining](./1-joining.mdx) guide. It's recommended that you use that instead.
:::
...
connected. Check out the for more details.
:::

:::tip
If you are using the **Steam** version of Mass Effect 3 you may run into connection issues if you're trying 
to play with others on Pocket Relay Client, if run into connection issues try using the [Plugin Client Guide](./4-plugin-client-manual.mdx) instead
:::
...
- **64bit Windows 10 or newer**
  ...
  - The client is supported on Linux however you may have to build it from source if your OpenSSL or GLIBC version is different
...
| <LinuxIcon /> Linux | [Download](https://github.com/PocketRelay/Client/releases/latest/download/pocket-relay-client-linux) |
...
file in order to redirect the game traffic see the [Client Technical Overview](../technical/client/hosts-file.md) to
