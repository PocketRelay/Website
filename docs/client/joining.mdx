...
connected. Check out the [Plugin Client Guide](plugin-client.mdx) for more details.
:::

:::tip
If you are using the **Steam** version of Mass Effect 3 you may run into connection issues if you're trying 
to play with others on Pocket Relay Client, if run into connection issues try using the [Plugin Client Guide](plugin-client.mdx) instead
:::
